...
class TMotorsEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # tau = action[0] * m * l * self.max_torque    
        tau = action[0] * m * l * 4.0                           # scale torque
        # max_torque_s = self.max_torque * m * l             # scale max torque
        max_torque_s = self.max_torque        # scale max torque
        tau = np.clip(tau, -max_torque_s, max_torque_s)

        self._apply_torque(float(tau))
        # time.sleep(0.015*np.sqrt(l/0.44))  # TODO: remove hardcode
        time.sleep(0.001)  # TODO: remove hardcode
        obs = self._get_obs()
        info = self._get_info()

        obs[2] = obs[2] / np.sqrt(1 / fac)                   # normalize theta_dot
    ...
    def dimensionless_cost(self, u):
        ...
        u_s = u * self.max_torque #/ (m * g * l)
        # costs = 1 - (q_s **2 * angle_normalize(th_s) ** 2 + (u_s**2)) / (q_s **2 * np.pi ** 2)

        costs = 1 - (angle_normalize(th_s) ** 2 + 0.1 * thdot_s**2 + 0.001 * (u_s**2)) / (np.pi ** 2)
        return costs
...
if __name__ == "__main__":
    ...
    max_torque_array = np.array([1.28423433, 2.07228901, 3.04514369])
    # env.max_torque = 4.0
    env.max_torque = max_torque_array[0]
    env.l = 0.37
    # env.m = 2*0.05 + 0.368 + 0.07 + 0.380*2
    # tube = 115g
